...
x = le2.fit_transform(train.loc[:, train.columns == 'PdDistrict'])

x = x.reshape(-1,1)

x_test = le2.fit_transform(test.loc[:, test.columns =='PdDistrict'])
x_test = x_test.reshape(-1,1)

y = le2.fit_transform(train.loc[:, train.columns == 'Category'])
...
pred_string = le2.inverse_transform(pred)
pred_string = pred_string.flatten('K')

test_id = test.loc[:, test.columns == 'Id']
test_id.append(pd.series(pred_string))

a = pd.DataFrame(test_id, pred_string[:])


pd.wide_to_long(a, i = 'Id', j = "value", stubnames = "fdjks")

submission = pd.DataFrame(pred, columns=le2.inverse_transform
